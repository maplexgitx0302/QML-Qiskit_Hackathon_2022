...
# Run qGAN
dir_path   = os.path.join(os.path.dirname(os.path.abspath(__file__)), f"stock")
...
for reps, hadamard, num_epochs in it.product(reps_list, h_list, epoch_list):
    ...
    for round in range(num_rounds):
        file_prefix = os.path.join(dir_path, f"result_U_H{hadamard}_stock_r{num_rounds}_e{num_epochs}_reps{reps}")
        ...
        axs[2].plot(
            np.cumsum(target_d), "-o", label="stock", color="deepskyblue", linewidth=4, markersize=12
        )
        ...
        axs[3].plot(
            target_d, "-o", label="stock", color="deepskyblue", linewidth=4, markersize=12
        )
